...
| `CacheDir`             | path     | `{DataDir}/cache`| Where probe results are persisted. Set to `none` to turn the disk tier off entirely (memory + Redis only). **On a multi-pod deployment with Redis, `none` is the recommendation**: Redis shares results between pods and surviving pods republish it from memory if it is emptied, so the disk tier earns little there. See [horizontal-scaling.md](horizontal-scaling.md) → *Skip the disk cache once Redis is present*. Blank resolves to the default, not to off. |
| `CacheTtlHours`        | int      | `2`              | TTL for the in-memory and on-disk DNS/SMTP/HTTP probe caches. `0` disables expiry: values never expire in memory and the load reads whatever is on disk. Files are **still swept after 24h**, and Redis keys still get a 24h lifetime, because neither tier can grow unbounded — see [caching-architecture.md](caching-architecture.md) → *Turning expiry off*. **Note the trade that makes:** memory keeps everything, but the two tiers that survive a restart are capped, so `0` comes back from a restart with *less* cache than an explicit `CacheTtlHours=168` would. Use `0` when you want a long-lived process never to re-fetch; use a large explicit value when you want restarts to stay warm. Two hours rather than a day because a flush writes only what it fetched since the last one, so a short retention costs little and bounds both file count and staleness. |
...
| `SharedCacheWatchSeconds` | int   | `30`             | How often each instance checks whether the shared Redis cache has been emptied and, if so, republishes its own in-memory cache into it. Redis-only; ignored otherwise. One `GET` per interval, so it is polled far more often than the disk flush — it bounds how long the shared cache stays cold after a Redis restart. `0` disables the watch and the key index it prunes, so re-enabling it takes a restart. |
| `DnsNegativeTtlCapSeconds` | int | `600` | Ceiling on how long a **negative answer** may be cached, however long the zone's SOA asks for. **This is a TTL for a positive non-result, not for errors:** the lookup *succeeded* and established that something does not exist — an absent DKIM selector, an IP with no PTR, a "not listed" blocklist reply, a name with no CAA. Timeouts and SERVFAIL are *failures*, not negative answers; they get a fixed 30s and never leave the pod. Applies whether or not `DnsCacheMinTtlSeconds` gating is on — it is a ceiling, not part of the gating — and `CacheTtlHours` remains the outer ceiling. Far below `CacheTtlHours` on purpose: a stale "exists" goes out of date, a stale "does not exist" becomes a false finding, so one spurious NXDOMAIN cannot become a two-hour, disk-persisted, fleet-shared error. `0` removes it. Rechecks bypass it like any cached read. See [cache-behaviour-map.md](cache-behaviour-map.md). |
| `DnsCacheMinTtlSeconds`| int      | `0`              | `0` = off: every cached DNS answer gets the full `CacheTtlHours`. Above zero, answers are bounded by the TTL the zone published, as `clamp(published TTL, this floor, CacheTtlHours)` — the minimum record TTL for a positive answer, or the RFC 2308 negative TTL (`min(SOA.MINIMUM, SOA TTL)`) for an NXDOMAIN/NODATA. **A floor, not a default:** a response that published no TTL at all inherits `CacheTtlHours` rather than this value. See [caching-architecture.md](caching-architecture.md) → *DNS record-TTL gating*. |
