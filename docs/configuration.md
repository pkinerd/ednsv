...
| `CacheTtlHours`        | int      | `2`              | TTL for the in-memory and on-disk DNS/SMTP/HTTP probe caches. `0` disables expiry: values never expire in memory and the load reads whatever is on disk. Files are **still swept after 24h**, because each flush appends a new one and nothing else would ever remove them — see [caching-architecture.md](caching-architecture.md) → *Turning expiry off*. Two hours rather than a day because a flush writes only what it fetched since the last one, so a short retention costs little and bounds both file count and staleness. |
