...
| `DnsNegativeTtlCapSeconds` | int | `600` | Ceiling on how long a **negative** answer (NXDOMAIN / NODATA) is cached, however long the zone's SOA asks for. Applies whether or not `DnsCacheMinTtlSeconds` gating is on — it is a ceiling, not part of the gating. Deliberately far below `CacheTtlHours`: a stale "exists" goes out of date, a stale "does not exist" becomes a false finding, so one bad NXDOMAIN cannot become a two-hour, disk-persisted, fleet-shared error. `0` removes it. |
