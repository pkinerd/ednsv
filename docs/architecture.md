...
| **ExpiringMap\<K,V\>** | The other cache primitive: a `MemoryCache` plus this project's three cache rules (recheck bypass, null TTL means no expiry, an import keeps its record's expiry), for the seven caches that are not a `ProbeCache` (RCPT probes, relay tests, AXFR verdicts and responses, unreachable-server counts, domain summaries, the per-server `LookupClient` pool). Expiry and its sweep are the platform's; a `lock` covers the one caller needing an atomic read-modify-write. |
