...
│   │       ├── ExpiringMap.cs         # Keyed map with TTL, for the caches that are not a ProbeCache
...
| **ExpiringMap\<K,V\>** | The other cache primitive: a `ConcurrentDictionary` of value-plus-expiry, for the seven caches that are not a `ProbeCache` (RCPT probes, relay tests, AXFR verdicts and responses, unreachable-server counts, domain summaries, the per-server `LookupClient` pool). Expiry is enforced on read; a thread-pool sweep, triggered by a write and rate-limited to one a minute, bounds memory — the same three decisions `MemoryCache` makes internally. Used where `MemoryCache` cannot go on .NET 8: atomic read-modify-write and enumeration of the live set. |
