...
rootCommand.SetHandler(async (string domain, string format, bool noAxfr, bool catchAll, bool openRelay, string[] dkimSelectors, bool listChecks, bool verbose) =>
{
    ...
    try
    {
        ...
        {
            case "json":
                await RunJsonAsync(domain, options, writer);
                break;
            case "html":
                await RunHtmlAsync(domain, options, writer);
                break;
            case "markdown":
            case "md":
                await RunMarkdownAsync(domain, options, writer);
                break;
            default:
...
static async Task RunJsonAsync(string domain, ValidationOptions options, TextWriter writer)
{
    var validator = new DomainValidator();
    var report = await validator.ValidateAsync(domain, options);
    ...
    await writer.WriteLineAsync(JsonSerializer.Serialize(report, jsonOptions));
}

static async Task RunMarkdownAsync(string domain, ValidationOptions options, TextWriter writer)
{
    var validator = new DomainValidator();
    var report = await validator.ValidateAsync(domain, options);
...
static async Task RunHtmlAsync(string domain, ValidationOptions options, TextWriter writer)
{
    var validator = new DomainValidator();
    var report = await validator.ValidateAsync(domain, options);
